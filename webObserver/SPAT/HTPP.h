...
public:
    ...
    static void getRequest(std::string address, std::function<void(std::string)> onResponce, std::string what = "#")
    {
        ...
        requestBase(address, "GET", [&](HINTERNET request)
            {
                std::wstring whatArg(what.begin(), what.end());
                std::wstring arg = L"arg: " + whatArg;
                return WinHttpSendRequest(request,
                    arg.c_str(),
                    -1L, WINHTTP_NO_REQUEST_DATA, 0,
                    0, 0);
            },
